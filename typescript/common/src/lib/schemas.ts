...
const PR_SONG_DOCUMENT = (game: GameGroup): PrudenceSchema => {
...
type GameSchemas = Record<`${"charts" | "songs"}-${GameGroup}`, SchemaValidatorFunction>;
...
const PRE_SCHEMAS = {
	...
	imports: prSchemaFnWrap({
		...
		playtypes: [(self) => p.isIn(GetGameGroupConfig(self.game).playtypes)(self)],
		errors: [
			{
				type: "string",
				message: "?string",
